...
func (pr *packedFileReader) newArchiveFileFrom(r archiveFile, blocks *fileBlockList) (archiveFile, error) {
	...
	if h.decVer > 0 {
		...
		// SWIRL fix: the first file of a solid archive is not marked solid, but the files after it
		// reuse its window, so the window may only be shrunk when the whole archive is not solid.
		if !h.Solid && !h.arcSolid && !h.UnKnownSize && h.winSize > h.UnPackedSize {
