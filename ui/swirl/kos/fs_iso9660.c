/* SWIRL: vendored from KallistiOS 2.2.1 with one change in init_percd() (high density area first). */
/* KallistiOS ##version##
...
static int init_percd(void) {
    ...
    /* SWIRL: read the GD-ROM high density area first. GDEMU menu discs built by
       GDMENUCardManager keep only OPENMENU.INI in the low density track and all
       art, metadata and themes in the high density area. CD-Rs fall back below. */
    session_base = 0;
    if(cdrom_read_toc(&toc, true) == 0) {
        /* The volume descriptors live at the start of the FIRST high density data track
           (track 3). GDMENUCardManager puts the file data in a last track at the end of the
           disc, so cdrom_locate_data_track() (which returns the last data track) is wrong here. */
        int t, first = TOC_TRACK(toc.first), last = TOC_TRACK(toc.last);
        if(first >= 1 && last <= 99 && first <= last) {
            for(t = first; t <= last; t++) {
                if(TOC_CTRL(toc.entry[t - 1]) == 4) {
                    session_base = TOC_LBA(toc.entry[t - 1]);
                    break;
                }
            }
        }
